// app/dashboard/branch/sections/page.tsx
// Purpose: Branch Admin — Section Management (create/manage classroom sections per grade/stream/year)
import {
  requireRole,
...
import SectionManagerClient from "@/components/admin/section-manager-client"

export default async function SectionManagementPage() {
  ...
  const { data: sections, error: sectionsError } = await adminClient
    .from("sections")
    .select("id, grade_id, stream_id, academic_year_id, name, max_capacity, is_active, grades(name, level_order), streams(name), academic_years(name)")
